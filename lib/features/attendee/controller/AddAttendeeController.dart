...
import '../model/AddAttendeeResponse.dart';
...
class AddAttendeeController extends GetxController {
  ...
  Rx<TextEditingController> dobDayController = TextEditingController().obs;
  Rx<TextEditingController> dobMonthController = TextEditingController().obs;
  Rx<TextEditingController> dobYearController = TextEditingController().obs;
  ...
  void onInit() {
    ...
    if (arguments != null) {
      ...
      if (arguments['Attendees'] != null) {
        ...
        if (data.fldDob.isNotEmpty) {
          try {
            DateTime? date;
            if (data.fldDob.contains('-')) {
              date = DateFormat('yyyy-MM-dd').parse(data.fldDob);
            } else if (data.fldDob.contains('/')) {
              // Try yyyy/MM/dd first then dd/MM/yyyy
              try {
                date = DateFormat('yyyy/MM/dd').parse(data.fldDob);
              } catch (e) {
                date = DateFormat('dd/MM/yyyy').parse(data.fldDob);
              }
            }
            if (date != null) {
              dobDayController.value.text = date.day.toString().padLeft(2, '0');
              dobMonthController.value.text = date.month.toString().padLeft(2, '0');
              dobYearController.value.text = date.year.toString();
              calculateAge();
            }
          } catch (e) {
    ...
    }GiftGivenController.value.text="T-Shirt";
    CallGetdata();
  }
  ...
  validateAndProcess() async {
    ...
    else if (dobDayController.value.text.isEmpty || dobMonthController.value.text.isEmpty || dobYearController.value.text.isEmpty) {
      AppUtils.showSnackbar(Get.context!, "Please enter complete Date of Birth", "Info");
    }
    else if (int.tryParse(dobDayController.value.text) == null || int.parse(dobDayController.value.text) < 1 || int.parse(dobDayController.value.text) > 31) {
      AppUtils.showSnackbar(Get.context!, "Please enter a valid Day (1-31)", "Info");
    }
    else if (int.tryParse(dobMonthController.value.text) == null || int.parse(dobMonthController.value.text) < 1 || int.parse(dobMonthController.value.text) > 12) {
      AppUtils.showSnackbar(Get.context!, "Please enter a valid Month (1-12)", "Info");
    }
    else if (int.tryParse(dobYearController.value.text) == null || int.parse(dobYearController.value.text) < 1958 || int.parse(dobYearController.value.text) > DateTime.now().year-17) {
      AppUtils.showSnackbar(Get.context!, "Please enter a valid Year (${DateTime.now().year - 17} or earlier)", "Info");
    }
    else if (!isValidDate(int.parse(dobDayController.value.text), int.parse(dobMonthController.value.text), int.parse(dobYearController.value.text))) {
      AppUtils.showSnackbar(Get.context!, "Please enter a valid Date", "Info");
    }
    else if (AgeController.value.text.trim().isEmpty) {
      AppUtils.showSnackbar(Get.context!, "Please check your Date of Birth", "Info");
  ...
  Future<void> CallNoteAdd_update() async {
    ...
    var requestBody = {
      ...
      "fld_dob": "${dobYearController.value.text}/${dobMonthController.value.text}/${dobDayController.value.text}",
...
"fld_project_type":selectedProjectType.value,
    ...
    productServices.Addattendee(body: requestBody, modify: Attendee_id != 0)
      ...
      if (value.success == true) {
        ...
        if (value.message!.contains('Please go back screen to update') ) {
          //AppUtils.showSnackbar(Get.context!, "Please wait we are", "Error");
          if (value.data != null && value.data!.fldAid != null) {
            Attendee_id = value.data!.fldAid!;
            CallNoteAdd_update();
          }
        }else{
  ...
  void calculateAge() {
    try {
      String dayText = dobDayController.value.text;
      String monthText = dobMonthController.value.text;
      String yearText = dobYearController.value.text;

      if (dayText.isNotEmpty && monthText.isNotEmpty && yearText.length == 4) {
